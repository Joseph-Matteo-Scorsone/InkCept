...
pub fn Actor(comptime TStruct: type) type {
    ...
    return struct {
        ...
        fn processMessage(self: *Self) void {
            ...
            defer msg.deinit(self.allocator); // Ensure message is deallocated even on error

            if (self.stop_flag.load(.acquire)) {
                return; // Skip processing if actor is stopped
            }

            self.t_struct.receive(self.allocator, msg) catch |err| {
                // Log the error (optional, can be customized based on needs)
                std.log.err("Actor {} failed to process message: {}", .{ self.actor_id, err });
                // Optionally, you could add recovery logic here, e.g., retry or notify
            };
        }
